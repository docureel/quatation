// Firebase Configuration (আপনার দেওয়া কনফিগারেশন)
...
document.addEventListener("DOMContentLoaded", () => {
    applyZoom();
    loadDashboard(); // Firebase থেকে ডাটা লোড করবে
...
function showSection(sectionId) {
    ...
    if(sectionId !== 'output-section') {
        ...
        if(sectionId === 'form-section') {
            ...
            loadDashboard(); // ড্যাশবোর্ড ট্যাবে গেলেই ডাটা রিফ্রেশ হবে
        }
    }
}
...
document.getElementById('quotation-form').addEventListener('submit', async function(e) {
    ...
    try {
        if (firebaseDocId) {
            ...
            quoteData.firebaseDocId = firebaseDocId; // Keep the ID for rendering
            
        } else {
            // Create New Document
            const today = new Date();
            
            // Serial Number Logic (Local + Firebase ready)
            ...
            quoteData.timestamp = firebase.firestore.FieldValue.serverTimestamp(); // For sorting in dashboard
...
async function loadDashboard() {
    ...
    try {
        // Fetch from Firebase (Order by Timestamp Descending)
        const snapshot = await db.collection("quotations").orderBy("timestamp", "desc").get();
        let history = [];
        snapshot.forEach(doc => {
            let data = doc.data();
            data.firebaseDocId = doc.id; // Keep Firebase Document ID
            history.push(data);
        });
...
async function deleteHistoryItem(docId) {
    if(confirm("Are you sure you want to delete this quotation PERMANENTLY from the database?")) {
        try {
            ...
            loadDashboard(); // Refresh Dashboard after delete
        } catch (error) {
